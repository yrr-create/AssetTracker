...
它的定位不是单纯点灯实验，而是做一个接近真实仓库管理场景的小型资产标签：手机可以扫描标签、连接标签、查看状态、触发查找，标签端可以用 LED 和蜂鸣器提示位置，并上报温湿度等环境信息。
...
下一步重点不是继续堆硬件，而是把手机端 App 做出来，让这个原型从 nRF Connect 验证工具变成一个真正能演示的仓库资产管理界面。
