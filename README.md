...
| 阶段 | 目标 | 预计时间 | 完成标准 |
| --- | --- | --- | --- |
| 第 1 | 编译通过 | 0.5-1 天 | 生成 hex/bin/out |
| 第 2 | SWD 烧录 | 0.5-1 天 | 能 erase/program/reset |
| 第 3 | GPIO 运行证明 | 0.5-1 天 | LED/GPIO/RTT 可观察 |
| 第 4 | BLE UART | 1-3 天 | 手机连接并收发文本 |
| 第 5 | 资产标签控制协议 | 1-2 天 | nRF Connect 能控制 LED/蜂鸣器并读取状态 |
| 第 6 | Android App MVP | 2-3 天 | App 扫描 L4 标签、显示 RSSI、连接并查找 |
...
1. 做第 1-3 时读：nRF52810 / `pca10040e` 说明，理解为什么不是 `pca10040`。
2. 做第 4 时读：nRF5 SDK BLE UART/NUS 示例文档。
3. 做第 5 时读：nRF5 SDK BLE NUS 示例、GPIO/GPIOTE、按键输入和 PWM 基础。
4. 做第 6 时读：Android BLE 扫描、连接、GATT characteristic 写入、RSSI 读取和运行时权限。
